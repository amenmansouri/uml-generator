define(function (require, exports, module) {
    ...
    var outputFilename = "AlgebraSystemASP.jh";
    var outputString = "";
     ...
     */
    function JdlGen(baseModel, basePath) {
     ...
     * @return {$.Promise}
     */
    JdlGen.prototype.generate = function (elem, path, options) {
...
//                this.writeAnnotationType(codeWriter, elem, options, isAnnotationType);
     ...
     * @param {Object} options
     */
    JdlGen.prototype.writeClass = function (codeWriter, elem, options) {
        var i, len, terms = [];
      ...
      /*var associations = Repository.getRelationshipsOf(elem, function (rel) {
        ...
        for (i = 0, len = associations.length; i < len; i++) {
           ...
           // codeWriter.writeLine(asso.end2.multiplicity)
     ...
     * @return {string}
     */
    JdlGen.prototype.getType = function (elem) {
    ...
    JdlGen.prototype.writeMemberVariable = function (codeWriter, elem, options) {
        ...
        JdlGen.prototype.writeAnnotation =function(codeWriter,elem1,elem2, options){
                
                   
            if (elem2.multiplicity === "0..*" || elem2.multiplicity === "1..*" ){
                codeWriter.writeLine("relationship OneToMany  {");
                codeWriter.writeLine()
                codeWriter.writeLine(this.getType(elem1) +"{"+ elem2.name +"} to " + this.getType(elem2) +" ")
                codeWriter.outdent()
                codeWriter.writeLine("}")
            }
            else
               if(elem2.multiplicity === "0..1" || elem2.multiplicity === "1..1")
                {
                    codeWriter.writeLine("relationship OneToOne {");
                    codeWriter.writeLine()
                    codeWriter.writeLine(this.getType(elem1) +"{"+ elem2.name +"} to " + this.getType(elem2) +" ")
                    codeWriter.outdent()
                    codeWriter.writeLine("}")      
               ...
               if(elem2.multiplicity === "*..1" || elem2.multiplicity === "*..0")
                {
                    codeWriter.writeLine("relationship ManyToOne {");
                    codeWriter.writeLine()
                    codeWriter.writeLine(this.getType(elem1) +"{"+ elem2.name +"} to " + this.getType(elem2) +" ")
                    codeWriter.outdent()
                    codeWriter.writeLine("}")      
        } else 
            if (elem1.multiplicity === "0..*" || elem1.multiplicity === "1..*" ){
           codeWriter.writeLine("relationship OneToMany  {");
           codeWriter.writeLine()
           codeWriter.writeLine(this.getType(elem2) +"{"+ elem1.name +"} to " + this.getType(elem1) +" ")
           codeWriter.outdent()
           codeWriter.writeLine("}")
       }
       else
          if(elem1.multiplicity === "0..1" || elem1.multiplicity === "1..1")
           {
               codeWriter.writeLine("relationship OneToOne {");
               codeWriter.writeLine()
               codeWriter.writeLine(this.getType(elem2) +"{"+ elem1.name +"} to " + this.getType(elem1) +" ")
          ...
          if(elem1.multiplicity === "*..1" || elem1.multiplicity === "*..0")
           {
               codeWriter.writeLine("relationship ManyToOne {");
               codeWriter.writeLine()
               codeWriter.writeLine(this.getType(elem2) +"{"+ elem1.name +"} to " + this.getType(elem1) +" ")
     ...
     * @param {Object} options
     */
    JdlGen.prototype.writeEnum = function (codeWriter, elem, options) {
        var i, len;
    ...
    function generate(baseModel, basePath, options) {
        var result = new $.Deferred();
        var jdlGen = new JdlGen(baseModel, basePath);
